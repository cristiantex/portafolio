...
<div class="bg-white rounded-lg shadow p-6 w-full min-h-[80vh]">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('proyectos.create') }}" class="px-2 py-1 bg-blue-500 text-white rounded shadow hover:bg-blue-600 transition">
    ...
    <table class="w-full border border-gray-300" id="table_proyectos">
        <thead>
        ...
        <tbody>
            ...
            <tr class="text-gray-700">
                ...
                <td class="border px-4 py-2">{{ $p->tecnologias ?? '-' }}</td>
                <td class="border px-4 py-2">
                    @if($p->url)
                        <a href="{{ $p->url }}" target="_blank" class="text-blue-500 hover:underline">{{ $p->url }}</a>
                    @else
                        -
                    @endif
                </td>
                <td class="border px-4 py-2 text-center">
                    @if($p->publicado)
                        <span class="px-2 py-1 font-bold text-green-800">Sí</span>
                    @else
                        <span class="px-2 py-1 font-bold text-red-800">No</span>
                    @endif
                </td>
                <td class="border px-4 py-2 text-center">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const table = document.querySelector("#table_proyectos");
        if (table) {
        ...
        const routeBase = "{{ route('proyectos.destroy', 0) }}";
