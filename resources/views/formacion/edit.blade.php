...
<div class="bg-white rounded-lg shadow p-6 w-full min-h-[80vh]">
    ...
    <h2 class="text-2xl font-bold mb-4 text-gray-800">Editar Formación</h2>
    ...
    <form action="{{ route('formacion.update', $formacion->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="tipo"
                ...
                <option value="Carrera" {{ old('tipo', $formacion->tipo) == 'Carrera' ? 'selected' : '' }}>Carrera</option>
                <option value="Diplomado" {{ old('tipo', $formacion->tipo) == 'Diplomado' ? 'selected' : '' }}>Diplomado</option>
                <option value="Curso" {{ old('tipo', $formacion->tipo) == 'Curso' ? 'selected' : '' }}>Curso</option>
                <option value="Certificación" {{ old('tipo', $formacion->tipo) == 'Certificación' ? 'selected' : '' }}>Certificación</option>
                <option value="Otro" {{ old('tipo', $formacion->tipo) == 'Otro' ? 'selected' : '' }}>Otro</option>
            </select>
            @error('tipo')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="date" name="fecha_inicio"
                   value="{{ old('fecha_inicio', $formacion->fecha_inicio) }}"
        ...
        <div>
            ...
            <input type="date" name="fecha_fin"
                   value="{{ old('fecha_fin', $formacion->fecha_fin) }}"
        ...
        <div>
            ...
            <input type="url" name="certificado_url"
                   value="{{ old('certificado_url', $formacion->certificado_url) }}"
